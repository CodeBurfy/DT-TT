...
const AddListingPage = () => {
  const [formData, setFormData] = useState({
    ...
    denomination: '',
    businessType: '',
    activityType: '', // New field for activity type
    schedule: '', // New field for activity schedule
    category: '',
    vendorIds: [],
    activityIds: [], // New field for selected activities
  });
  const [imageFile, setImageFile] = useState(null);
  const [imagePreview, setImagePreview] = useState(null);
  const [categories, setCategories] = useState([]);
  const [vendors, setVendors] = useState([]);
  const [activities, setActivities] = useState([]); // New state for available activities
